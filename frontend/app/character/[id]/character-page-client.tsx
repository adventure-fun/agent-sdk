...
export function CharacterPageClient({ id }: { id: string }) {
  ...
  // Status-aware share text — celebrates a live run or memorializes a fall.
  const classTitle = titleCase(character.class)
  const shareUrl = `${SITE_URL}/character/${character.id}`
  const shareText = isAlive
    ? `Following ${character.name}'s run on Adventure.fun — Lv ${character.level} ${classTitle}. One life. Real stakes.`
    : `${character.name} (${classTitle}) fell${data.history?.cause_of_death ? ` to ${data.history.cause_of_death}` : ""}. Read their legend on Adventure.fun.`
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 py-12 md:py-16 space-y-8">
        ...
        <section className="flex items-start gap-6">
          ...
          <div className="flex flex-col items-end gap-3 shrink-0">
            ...
            <ShareCard
              url={shareUrl}
              title={`${character.name} — Adventure.fun`}
              text={shareText}
              hashtags={["AdventureFun"]}
              size="sm"
              tone={isAlive ? "amber" : "error"}
            />
          </div>
        </section>
