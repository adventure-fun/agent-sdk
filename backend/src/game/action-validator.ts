...
import type { Action, EquipSlot } from "@adventure-fun/schemas"
...
/**
 * Checks whether `action` structurally matches at least one entry in `legalActions`.
 *
 * For attack actions without an explicit ability_id, we match against
 * legal actions with ability_id "basic-attack" (the engine default).
 */
