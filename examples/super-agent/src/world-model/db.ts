...
/**
 * Opens (or creates) the WorldModel SQLite database at `path`. Applies the schema idempotently,
 * enables WAL mode so docker SIGKILL doesn't corrupt the file, and returns a ready-to-use
 * Database instance. Use `":memory:"` for tests.
 */
export function openWorldDatabase(path: string): Database {
  ...
  if (path !== ":memory:") {
    try {
      ...
      // WAL is best-effort. Some filesystems reject it; fall through to the default journal.
    }
  }
  db.exec(SCHEMA)
  return db
