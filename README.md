...
Seven reference agents, in increasing order of complexity. Each is self-contained — `config.ts` + `index.ts` + (optionally) `supervisor.ts` + a `tests/` directory — so you can copy a whole folder as the starting point for your own agent. The `super-agent` and `hybrid-agent` examples ship a `supervisor.ts` crash-loop wrapper (exponential backoff 2s → 60s, SIGTERM/SIGINT graceful shutdown) intended as the Docker entrypoint.

Two flavors of example agent coexist:

- **LLM-enabled** (`basic-agent`, `strategic-agent`, `super-agent`, `arena-agent`, `hybrid-agent`) — the full decision pipeline with OpenRouter/OpenAI/Anthropic tactical models. Best when you want adaptive, conversational, per-run behavior.
- **Deterministic** (`deterministic-arena`, `deterministic-realm`) — module-only fleets that reuse the exact same module code but swap in a `NullLLMAdapter` and force `decision.strategy = "module-only"`. Intended for always-on fleet coverage where you don't want to burn API credits, but still want real gameplay variance from `BotArchetype` personality knobs. Both deterministic examples are the runtimes the `bot-agents/docker-compose.deterministic.yml` fleet uses.
...
- [`examples/deterministic-realm/`](examples/deterministic-realm/) -- zero-LLM drop-in for `super-agent`. Reuses the full super-agent module roster but forces `decision.strategy = "module-only"`, replaces the LLM adapter with `NullLLMAdapter`, disables chat, and swaps in a `DeterministicNameProvider` (stable names across rerolls). `maxSpendUsd = 0` so it refuses to hit any paid endpoint. See its [README](examples/deterministic-realm/README.md).

### Arena agents (4-player PvP)

- [`examples/arena-agent/`](examples/arena-agent/) -- pure arena example. Authenticates, calls `POST /arena/queue`, polls `GET /arena/queue/status` for a `match_id`, then connects to `GET /arena/match/:id/play` and pumps arena-specific modules (`arena-cowardice-avoidance`, `arena-combat`, `arena-positioning`, `arena-chest-looter`, `arena-wave-predictor`, `arena-self-care`) through a module-first decision loop. Uses `BotArchetype` (`aggressive` / `balanced` / `cautious` / `opportunist`) to shift combat bias, heal trigger HP, and chest greed. Deliberately does **not** use `BaseAgent` — arena matches have no lobby / realm-progression loop, so the dungeon pipeline would be dead code. Bracket is selected via `ARENA_BRACKET=rookie|veteran|champion`. See its [README](examples/arena-agent/README.md) for the module priority ladder and prompt layout.
- [`examples/deterministic-arena/`](examples/deterministic-arena/) -- zero-LLM arena runner. Same module pipeline as `arena-agent` but never calls OpenRouter. Each turn the highest-confidence module wins; when nothing fires it emits `{ type: "wait" }`. Archetype + aggression knobs (`BOT_ARCHETYPE`, `BOT_AGGRESSION`) drive behavior variety instead of per-model prompting. See its [README](examples/deterministic-arena/README.md).
...
All examples share the same `.env` schema — switching between them is a one-line change to the `bun run …` target. The deterministic examples additionally honour `BOT_ARCHETYPE` / `BOT_AGGRESSION` for personality tuning and set `LLM_API_KEY=""` safely (the `NullLLMAdapter` throws instead of making a network call if a module ever misbehaves and requests LLM arbitration).
